...
function getColoredValue(value: number): string {
  if (value < 20) return chalk.red(`${value}€ (Ganga)`);
  if (value < 40) return chalk.yellow(`${value}€ (Económico)`);
  if (value < 70) return chalk.blue(`${value}€ (Precio estándar)`);
  ...
  .command('add', 'Añade un nuevo videojuego', gameOptions, async (argv) => {
    try {
      const game = new Videogame(
        argv.id, argv.name, argv.desc, argv.platform as Platform, argv.genre as Genre,
        argv.developer, argv.year, argv.multiplayer, argv.hours, argv.value
      );
